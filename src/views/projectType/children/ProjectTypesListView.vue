...
import ProjectTypeDetailDrawer from '@/components/detail/ProjectTypeDetailDrawer.vue'
...
function patchQuery(patch: Partial<ProjectTypeQueryParams>) {
  ...
  pendingQuery = next
  void router.replace({ query: { ...projectTypeQueryToRoute(next), ...detailQueryParam.value } }).finally(() => {
    pendingQuery = null
  })
}
...
const { data: selectedProjectType, isPending: detailLoading } = useQuery({
  queryKey: computed(() => ['projectTypes', 'detail', detailId.value]),
  queryFn: () => getProjectTypeDetailAction(detailId.value as number),
...
function openCreateForm() {
  router.push({ name: ROUTE.PROJECT_TYPE_NEW })
}

function openEditForm(item: { id: number }) {
  router.push({ name: ROUTE.PROJECT_TYPE_EDIT, params: { id: String(item.id) } })
}
