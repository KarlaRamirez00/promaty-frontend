...
import ClientDetailDrawer from '@/components/detail/ClientDetailDrawer.vue'
...
function patchQuery(patch: Partial<ClientQueryParams>) {
  ...
  pendingQuery = next
  void router.replace({ query: { ...clientQueryToRoute(next), ...detailQueryParam.value } }).finally(() => {
    pendingQuery = null
  })
}
...
const { data: selectedClient, isPending: detailLoading } = useQuery({
  queryKey: computed(() => ['clients', 'detail', detailId.value]),
  queryFn: () => getClientDetailAction(detailId.value as number),
...
function openCreateForm() {
  router.push({ name: ROUTE.CLIENT_NEW })
}

function openEditForm(item: { id: number }) {
  router.push({ name: ROUTE.CLIENT_EDIT, params: { id: String(item.id) } })
}
